...
public class TaskManager {
    ...
    public static boolean cancelTasks(Activity owner) {
        ...
        Log.w(Constants.TAG, String.format("cancelTasks: cancelling tasks for owner %s", key));
    ...
    public static void killIfNeeded(Task<?, ?> task) {
        if (task != null && task.isAlive()) {
            Log.d(Constants.TAG, String.format("about to cancel: %s", task.getClass().getCanonicalName()));
            task.cancel(true);
        }
    }
}
